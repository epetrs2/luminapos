...
import { Search, Plus, Clock, CheckCircle, Package, ArrowRight, X, Printer, Edit2, ListChecks, CheckSquare, Square, FileEdit, Receipt, GripVertical, User, Save, Minus, Scan, Trash2, Layers, PackagePlus, AlertTriangle, Timer, FileText, ArrowUpRight } from 'lucide-react';
...
const OrderCard = React.memo(({ 
    ...
    return (
        <div 
            ref={cardRef}
            className={`bg-white dark:bg-slate-800 p-4 rounded-xl border-l-4 ${statusColor} shadow-sm group transition-all relative select-none order-card-item will-change-transform hover:shadow-md`}
            draggable
            onDragStart={(e) => onDragStart(e, order.id)}
        >
            {/* Mobile Drag Handle */}
            <div 
                className="absolute top-0 right-0 p-2 w-14 h-14 flex justify-end items-start z-10 cursor-grab active:cursor-grabbing text-slate-300 dark:text-slate-600 opacity-100 md:opacity-0 group-hover:opacity-100 transition-opacity"
            ...
            <div className="flex justify-between items-start mb-2 pr-10 relative z-20">
                <div>
            ...
            {order.notes && (
                <div className="bg-yellow-50 dark:bg-yellow-900/10 p-2 rounded-lg mb-3 border border-yellow-100 dark:border-yellow-900/20 pointer-events-none">
            ...
            <div className="flex justify-between items-center gap-2 mt-auto relative z-30">
                <div className="flex gap-1" onTouchStart={(e) => e.stopPropagation()} onMouseDown={(e) => e.stopPropagation()}>
                    ...
                    {/* SURPLUS BUTTON NEXT TO TRASH */}
                    {onRegisterSurplus && (
                        <button 
                            onClick={onRegisterSurplus} 
                            className="p-1.5 bg-emerald-50 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400 hover:text-emerald-700 rounded transition-colors border border-emerald-100 dark:border-emerald-800" 
                ...
                {isReady ? (
                    <button 
                        ...
                        className="flex items-center gap-1.5 bg-emerald-600 hover:bg-emerald-700 text-white px-3 py-1.5 rounded-lg text-xs font-bold transition-all shadow-md hover:shadow-lg active:scale-95 cursor-pointer"
                    >
                    ...
                    <button 
                        ...
                        onMouseDown={(e) => e.stopPropagation()}
                        onTouchStart={(e) => e.stopPropagation()} 
                        className="flex items-center gap-1.5 bg-slate-900 hover:bg-slate-800 dark:bg-slate-700 dark:hover:bg-slate-600 text-white px-3 py-1.5 rounded-lg text-xs font-bold transition-colors shadow-sm md:hidden active:scale-95 cursor-pointer"
                    >
...
export const Orders: React.FC<OrdersProps> = ({ setView }) => {
    ...
    // Delivery & Surplus State
    const [deliveryModalOpen, setDeliveryModalOpen] = useState(false);
    const [deliveryOrder, setDeliveryOrder] = useState<Order | null>(null);

    // Explicit Surplus State
    ...
    const handleDeliveryClick = (orderId: string) => {
        ...
        if (order) {
            ...
            setDeliveryModalOpen(true); // Explicitly open modal
        }
    };

    // --- NEW SURPLUS FLOW ---
    const handleRegisterSurplusClick = (orderId: string) => {
        const order = orders.find(o => o.id === orderId);
        if (order) {
    ...
    const handleConfirmSurplus = (surplusItems: any[]) => {
        if (surplusOrder && surplusItems.length > 0) {
            registerProductionSurplus(surplusOrder.id, surplusItems);
    ...
    const handleScanSuccess = (decodedId: string) => {
        ...
        if (order.status === 'PENDING') {
            ...
            // Deliver - Trigger the same flow as the button
    ...
    return (
        <div className="p-4 md:p-8 pt-20 md:pt-8 md:pl-72 bg-slate-50 dark:bg-slate-950 min-h-screen transition-colors duration-200">
            ...
            {/* DELIVERY MODAL (SURPLUS CHECK) */}
            {deliveryModalOpen && deliveryOrder && (
                <DeliveryModal 
                    order={deliveryOrder} 
