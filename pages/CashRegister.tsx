            ...
            <div className="bg-slate-50 dark:bg-slate-800/50 rounded-2xl p-6 border border-slate-200 dark:border-slate-800">
                 ...
                 <form onSubmit={handleSubmit} className="space-y-4">
                    <div className="grid grid-cols-2 gap-2">
                         ...
                         {/* NEW: INVESTMENT WITHDRAWAL */}
                         <button type="button" onClick={() => { handleTypeChange('WITHDRAWAL', 'OTHER'); setSubCategory('Retiro Inversión'); }} className={`py-2 rounded-lg text-xs font-bold border transition-all ${type === 'WITHDRAWAL' && category === 'OTHER' && subCategory === 'Retiro Inversión' ? 'bg-cyan-100 border-cyan-300 text-cyan-800 dark:bg-cyan-900/40 dark:text-cyan-300' : 'bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700'}`}>Retiro Inversión</button>
                         {/* REEMBOLSO */}
